...
const cwd = process.cwd(),
  ...
  [filename, output, inputWidth, inputHeight] = args,
  { createCanvas, loadImage } = Canvas,
  TARGET_RATIO = calculateRatio(+inputWidth || 6, +inputHeight || 4);

if (args.length < 1) {
  console.log(
    "Usage: node index.js <file/folder> <output folder> [width] [height]"
  );
  process.exit(1);
}
...
function getImage(fileName: string): Promise<Buffer> {
...
async function resizeImage(
  ...
  const { width, height } = sizeOf(image),
    ratioWidthHeight = calculateRatio(width, height),
    ratioHeightWidth = calculateRatio(height, width),
    canvasImage = await loadImage(image),
    ratioCompareWidthHeight = ratioWidthHeight / TARGET_RATIO,
    ratioCompareHeightWidth = ratioHeightWidth / TARGET_RATIO,
    changeRequiredHeight = ratioCompareWidthHeight * height - height,
    changeRequiredWidth = ratioCompareHeightWidth * width - width;
  ...
  if (changeRequiredWidth >= 0 && (changeRequiredWidth <= changeRequiredHeight || changeRequiredHeight <= 0)) {
    ...
    // both are negative!
    const absoluteChangeRequiredWidth = Math.abs(changeRequiredWidth),
      absoluteChangeRequiredHeight = Math.abs(changeRequiredHeight);
  ...
  await fs.writeFile(
    path.resolve(outputFile, path.parse(fileName).name + ".png"),
    canvas.toBuffer()
  );
}

const inputFile = path.resolve(cwd, filename),
  outputFile = path.resolve(cwd, output),
  inputFileStat = await fs.stat(inputFile);
...
if (inputFileStat.isDirectory()) {
  const files = await fs.readdir(inputFile),
    images = await Promise.all(
      files.map(async (file) => {
        const filePath = path.resolve(inputFile, file),
  ...
  const image = await getImage(inputFile),
